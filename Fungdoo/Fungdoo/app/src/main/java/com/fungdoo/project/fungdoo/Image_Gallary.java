...
public class Image_Gallary extends Fragment {
    ...
    private void Setviwe() {
        ...
        for (int i = 0; i < File_Confix_Data.getOutput_data_List.get(File_Confix_Data.position_id).getImage_product().size(); i++) {

            ImageModel imageModel = new ImageModel();
            imageModel.setName(File_Confix_Data.getOutput_data_List.get(File_Confix_Data.position_id).getTravel_name()+"  "    );
            imageModel.setUrl(File_Confix_Data.getOutput_data_List.get(File_Confix_Data.position_id).getImage_product().get(i).toString());
